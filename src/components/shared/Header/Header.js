...
export default function Header() {
    ...
    return (
        <header id={shadow && "shadow"} className={styles.header}>
            ...
            <div className={styles.logo_wrapper}>
                <Link to="/" style={{ display: 'flex' }}>
                    ...
                    <p className={styles.logo_name}>SwiftQuiz</p>
                </Link>
            </div>

            <div className={styles.links_wrapper}>
                <div className={styles.links}>
                    <Link to="/f" className={styles.link}>{t("header.mission")}</Link>
                    <Link to="/f" className={styles.link}>{t("header.features")}</Link>
                    <Link to="/f" className={styles.link}>{t("header.team")}</Link>
                    <Link to="/f" className={styles.link}>{t("header.contact")}</Link>
                </div>
                {
                    auth.token === undefined ?
                        <div className={styles.buttons}>
                            <Link to="/auth" className={styles.btn_secondary}>
                                {t("header.login")}
                            </Link>

                            <Link to="/auth" className={styles.btn_primary}>
                            {t("header.try")}
                            </Link>
                        </div>
                        :
                        <div className={styles.buttons}>
                            <button onClick={() => dispatch(logOutAction())} className={styles.btn_secondary}>
                            {t("header.logout")}
                            </button>

                            <Link to="/dashboard/quizzes" className={styles.btn_primary}>
                               {t("header.dashboard")}
                                <img
                ...
                <div className={styles.menumobile}>
                    <h2 onClick={() => setIsMobileModalOpen(!isMobileModalOpen)}>open me</h2>
                    {isMobileModalOpen &&
                        <div className={styles.mobilemodal}>
                            <div>
                                <Link to="/f" className={styles.link}>Our Mission</Link>
                                <Link to="/f" className={styles.link}>Features</Link>
                                <Link to="/f" className={styles.link}>Our Team</Link>
                                <Link to="/f" className={styles.link}>Get in Touch</Link>
                            </div>
                            <div>
                                <Link to="/auth" className={styles.btn_secondary}>
                                    Sign in
