...
export default function Quiz() {
    ...
    useEffect(() => {
        ...
        setQuiz(selected_quiz)
        setQuestionList(selected_quiz.questions)
        setIsFetching(false);
    }, [])
    ...
    function checkValidation() {
        ...
        if (quiz.questions.length === 0) errors.no_questions = 'You should add at least 1 question with anwsers.';

        quiz.questions.map(
            item => {
                if (item.question === '') errors.question = 'All questions should be filled'
                ...
                if (item.isCorrect === '') errors.isCorrect = 'All Correct answers must be chosen'
            }
        )
        ...
        if (isEmpty(errors) === true || isEmpty(errors) === null) {
            ...
            message.title = "Ops! 🥺"
            message.description = "We couldn't publish your quizzes because there are some fields that aren't filled yet."
            setNotification(true);
            setTimeout(function () {
                setNotification(false);
            }, 5000)
                        ...
                        {questionList.length <= 0 ? (
                            ...
                            <>
                                {questionList.map((item, i) => {
                                    return (
                                        <div key={i} className={item.quiz_id !== item.quiz_id ? styles.single_slide_active : styles.single_slide}>
                                            {item.question}
                        ...
                        {
                            {
                                ...
                                'quiz_edit':
                                    <div className={styles.goback} onClick={() => setSidebarView('question_operations')}>
                                        ...
                                        <svg className={styles.gobackbtn} style={{ marginLeft: '.5em' }} width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
